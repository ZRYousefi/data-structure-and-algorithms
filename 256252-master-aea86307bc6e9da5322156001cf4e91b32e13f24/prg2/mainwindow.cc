...
//    ui->output->appendPlainText("Update view:");
    ...
    for (auto& townid : towns)
    {
        ...
        auto dotitem = gscene_->addEllipse(20*x, -20*y, 8, 8, QPen(), QBrush(color));
...
//            auto lineitem = gscene_->addLine(20*x+4, -20*y+4, 20*vx+4, -20*vy+4, pen);
            auto pen = QPen(linecolor);
            pen.setWidth(0); // "Cosmetic" pen
            auto arrow = new Arrow(QPointF(20*x+4, -20*y+4), QPointF(20*vx+4, -20*vy+4));
            arrow->setPen(pen);
...
Arrow::Arrow(QPointF s, QPointF e, QGraphicsItem* parent)
    : QGraphicsLineItem(parent), start(s), end(e)
{
}
